const json_data=__DATA__;const username=json_data['name'];const cover_img=json_data['cover-image'];const avatar_img=json_data['avatar-image']
const skills=json_data['skills'];const bio_storys=json_data['bio-storys']
const links=json_data['links']
const page_title=json_data['config']['title'];const page_favicon=json_data['config']['favicon'];const is_tick=json_data['is_tick'];$(document).ready(function(){console.log('READY !')
draw_console_header();var container=document.getElementsByClassName('container')[0];var bio_story_html='';var skills_html='';var links_html='';document.title=page_title;document.getElementById('favicon').setAttribute('href',`./public/images/${page_favicon}`);show_modal(json_data['config']['notice']);var split_name=username.split();var first_name='';var last_name='';if(split_name.length<=2){first_name=split_name.join(' ');}else if(split_name==3){first_name=split_name[0]+split_name[1];last_name=split_name[2];}else if(split_name==4){first_name=split_name[0]+split_name[1];last_name=split_name[2]+split_name[3];}
if(is_tick){first_name+='<i class="fas fa-check-circle"></i>';}
...
document.getElementById("dismiss-popup-btn").addEventListener("click",function(){document.getElementsByClassName("popup")[0].classList.remove("active");});function draw_console_header(){}